...
public class C_Queue {
    ...
    public static String get_next_queue_task() {
        ...
        ConfigurationSection queue = null;
        try {
        ...
        Object[] queue_temp = null;
        String[] queue_sort = null;
        try {
        ...
        for (int i = 0; i < queue_temp.length; i++) {
            try {
                ...
                try {
                    if (is_queue_open(idx)) {
                        queue_sort[i] = (time.trim() + "," + idx.trim() + "," + active.trim());
                    } else {
        ...
        if (task != null) {
            ...
            for (int i = 0; i < task_temp.length; i++) {
                try {
                    ...
                    time = VA_postal.plugin.getConfig().getString(path);
                    if (is_task_open(selected_queue + "," + idx)) {
                        task_sort[i] = (time.trim() + "," + idx.trim());
                    } else {
    ...
    public static synchronized String get_queue_pair(String town, String address) {
        ...
        String t_address = "";
        String path = GetConfig.path_format("dispatcher.queue");
        ConfigurationSection queue_list = null;
        try {
            queue_list = VA_postal.configsettings.getConfigurationSection(path);
        } catch (Exception e) {
            return "null";
        }
        Iterator<String> itr_queue_list_keys = null;
        try {
        ...
        while (itr_queue_list_keys.hasNext()) {
            ...
            Iterator<String> itr_task_list_keys = null;
            try {
            ...
            while (itr_task_list_keys.hasNext()) {
                ...
                try {
                    ...
                    if ((town.equalsIgnoreCase(t_town)) && (address.equalsIgnoreCase(t_address))) {
                        return qpair;
                    }
                } catch (Exception e) {
                    return "null";
    ...
    public static synchronized void queue_pair_set_task_age(String pair, String age, String interval) {
        ...
        try {
            ...
            if (!is_task_promoted(q_idx + "," + t_idx)) {
                VA_postal.plugin.getConfig().set(spath, age);
            }
    ...
    public static synchronized boolean queue_pair_activity_flag(String pair, boolean busy, boolean forward, boolean time_stamp) {
        ...
        if ("null".equals(scnt)) {
            VA_postal.plugin.getConfig().set(spath, "1");
        } else {
            try {
            ...
            for (int i = 0; i < task_cnt; i++) {
                ...
                if (is_task_defined(queue, task)) {
                    ...
                    if (VA_postal.dynmap_configured) {
                        P_Dynmap.delete_addr_label(stown, saddress);
                    }
    ...
    public static synchronized String get_null_or_next_queue() {
        ...
        if (new_queue.equals("null")) {
            new_queue = Util.int2str(q_cnt);
        }
        return new_queue;
    }
    ...
    public static synchronized String get_null_or_next_task(String queue) {
        ...
        if (new_task.equals("null")) {
            new_task = Util.int2str(t_cnt);
        }
        return new_task;
    }
}
